# Commercial Maintenance Billing Logic
...
## STEP 4.6 – RateAndServiceTax (Commercial)
...
3. **IF** Plot_Category = "Residential" → call **RatesResidential()** → exit (see ResidentialBillLogic.md)
4. **IF** Plot_Category = "Commercial" → call **RatesCommercial()** → exit

### Commercial Rate Lookup Key

When category routing falls through to the general fallback:

    Key = {Plot_Category} + "-" + {Plot_Size}

    IF Block contains "commercial" (case-insensitive):
        Key = "Commercial-1 M"
        IF Plot_Size is not empty THEN
            Key = {Plot_Category} + "-" + {Plot_Size}
...
### ServiceTaxGovt Rounding Rule (Commercial)

- For Commercial category, ServiceTaxGovt is **always rounded to nearest 100**
  (This differs from Residential where standard sizes 5M, 4M, 3M, 1M are exempt from rounding.)
...
ServiceTaxGovt   ← from Tariff/Rates Setup (always rounded to nearest 100 for Commercial)
...
|-------------------------------|-----------------------------------------------|-----------------------------------------------|
| Rate function called          | RatesResidential()                            | RatesCommercial()                             |
| ServiceTaxGovt rounding       | Only for non-standard sizes (not 5M/4M/3M/1M) | **Always** rounded to nearest 100            |
| Fallback rate key             | {Plot_Category}-{Plot_Size}                   | "Commercial-1 M" (or {Category}-{Size} if Plot_Size present) |
...
10. **ServiceTaxGovt Rounding**: Commercial always rounds to nearest 100 (unlike Residential which exempts standard sizes).
11. **Commercial Block Fallback**: If Block name contains "commercial" but no Plot_Size → defaults to key "Commercial-1 M".
